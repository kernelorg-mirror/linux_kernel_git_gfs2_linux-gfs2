...
struct gfs2_extent_header {
	__be16 eh_entries;	/* number of extents or extent indexes */
	__be16 eh_pad;
};

struct gfs2_extent_idx {
	__be64 ei_start;	/* offset in inode (blocks) */
	__be64 ei_leaf;		/* index or leaf block number */
} __attribute__((packed, aligned(4)));

#define GFS2_EF_UNWRITTEN	1

struct gfs2_extent {
	__be64 ex_start;	/* offset in inode (blocks) */
	__be64 ex_addr;		/* data block number */
	__be16 ex_len;		/* length (in blocks) */
